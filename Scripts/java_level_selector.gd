	...
	match level_number:
		1:
			...
			if LevelCore.html_mini_quiz_completed_2:
				...
				get_tree().change_scene_to_file("res://Java Scenes/java1.3.tscn")
			elif LevelCore.html_mini_quiz_completed:
				# User completed mini quiz, go to part 2
				get_tree().change_scene_to_file("res://Java Scenes/java1.2.tscn")
			else:
				# Start from the beginning
				get_tree().change_scene_to_file("res://Java Scenes/java tutorial start.tscn")
		2:
